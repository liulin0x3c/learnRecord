...
- 可达性分析算法
  - GC Roots
    - 栈中引用，锁持有，方法区中类的静态，常量引用，虚拟机常驻：装箱，异常，系统类加载器，
    - 本地方法栈中JNI，虚拟机栈（栈帧中的本地变量表）中引用的对象，
    - 方法区中类静态属性引用的对象，常量引用的对象（字符串常量池（String Table）的引用）
    - 同步锁（synchronized关键字）持有的对象
    - 虚拟机内部的引用：基本数据类型对应的Class对象，一些常驻的异常对象（比如 NullPointExcepiton、OOM）等，还有系统类加载器。
    - 局部回收需要存储的其他区域的引用。
- 引用
  - 强引用（Strongly Re-ference）
  - 软引用（Soft Reference） OOM前触发GC GC 软引用
  - 弱引用（Weak Reference） 下次GC直接杀
  - 虚引用（Phantom Reference）唯一目的只是为了能在这个对象被收集器回收时收到一个系统通知
- finalize（）
  - 执行条件：没有被引用 + 重写 + 第一次执行
  - F-Queue里慢慢执行，只保证运行，不保证运行结束
  - finalize（）执行时重新引用本身后被拯救

- 要判定一个类型是否属于“不再被使用的类”：
  - 实例回收
  - 类加载器回收
  - java.long.Class 没有被引用，不能被反射创建
- 标记-清除算法
  - 标记和清除两个过程的执行效率都随对象数量增长而降低
  - 内存空间的碎片化问题，CMS 碎片多了就标记-整理算法
  - 延迟低，没有咋瓦鲁多
- 标记-复制算法
  - Appel式回收 HotSpot中的这种分代方式
    - Eden:Survivor:Survivor = 8:1:1
    - 每次用作内存分配的空间是（8+1 = 9）90%
